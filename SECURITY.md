...
## `|raw` on JSON Configuration in JavaScript Context

**Files:** `views/twig/payment/adyen_assets_configuration.html.twig`, `views/twig/payment/adyen_assets.html.twig`
**Risk:** Medium

JSON-encoded configuration (containing user data like email, name, address) is output with `|raw` in a JavaScript context. While `json_encode()` is inherently XSS-safe, the module applies regex post-processing (removing outer braces, unquoting keys) that makes the pattern fragile.
